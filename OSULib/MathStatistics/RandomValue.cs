...
namespace OSULib.MathStatistics
{
    /// <summary>
    ...
    public static class MathStatistics
    {
        ...
        {
            int RowIndex = Misc.Misc.Data.FCCritical.Keys.OrderBy(rowindex => Math.Abs(rowindex - FreedomDegreeFirst)).First();
            int ColumnIndex = Misc.Misc.Data.FCCritical.First().Value.Keys
                .OrderBy(columnindex => Math.Abs(columnindex - FreedomDegreeSecond)).First();
            return Misc.Misc.Data.FCCritical[RowIndex][ColumnIndex];
        }
        ...
        public static double LaplasFunc(double Source)
        {
            ...
            if (Source >= 5)
                Result = 0.5;
            else if (Misc.Misc.Data.LaplasTable.ContainsKey(Source))
                Result = Misc.Misc.Data.LaplasTable[Source];
            else
            {
                double[] Keys = Misc.Misc.Data.LaplasTable.Keys.OrderBy(val => val).ToArray();
                ...
                Result = Misc.Misc.Data.LaplasTable[Lower] +
                         (Misc.Misc.Data.LaplasTable[Higher] - Misc.Misc.Data.LaplasTable[Lower]) * ((Source - Lower) / (Higher - Lower))
                    ;
            }
        ...
        {
            if (FreedomDegree > Misc.Misc.Data.CSCCritical.Keys.Max())
                return 2;
            return Misc.Misc.Data.CSCCritical[FreedomDegree];
        }
        ...
        /// <param name="freedomDegree"></param>
        /// <returns></returns>
        public static double GetChiSquaredCritical(int freedomDegree) => Misc.Misc.Data.ChiSquaredCriticals[freedomDegree - 1];
    };
}