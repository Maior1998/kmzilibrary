...
namespace OSULib.BinaryFunctions
{
    public partial class BinaryFunction
    {
        ...
        private static bool[][] GetFourierTransformFormula(BinaryFunction Source)
        {
            ...
            for (int i = 0; i < Source.ValuesArray.Length; i++)
            {
                if (!Source.ValuesArray[i])
                    continue;
                FourierTransformFormula.Add(Misc.Misc.GetBinaryArray(i, Source.CountOfVariables));
            }
        ...
        public static bool IsStable(BinaryFunction Source, int m)
        {
            ...
            for (int i = 0; i < (int)Math.Pow(2, Source.CountOfVariables); i++)
            {
                ...
                for (int j = 0; j < Source.ValuesArray.Length; j++)
                {
                    ...
                    bool[] CurrentFuncSet = Misc.Misc.GetBinaryArray(j, Source.CountOfVariables);
        ...
        private static (bool[], bool)[] GetWalshHadamardTransformFormula(BinaryFunction Source)
        {
            ...
            for (int i = 0; i < Source.ValuesArray.Length; i++)
                FourierTransformFormula.Add((Misc.Misc.GetBinaryArray(i, Source.CountOfVariables), Source.ValuesArray[i]));
        ...
        public static bool GetValue(BinaryFunction Source, bool[] Set)
        {
            ...
            return Source.ValuesArray[
                Enumerable.Range(0, (int)Math.Pow(Source.CountOfVariables, 2)).First(val => val == Misc.Misc.GetValue(Set))];
        }
