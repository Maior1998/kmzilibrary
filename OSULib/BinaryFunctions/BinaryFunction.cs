...
namespace OSULib.BinaryFunctions
{
    /// <summary>
    ...
    public partial class BinaryFunction
    {
        ...
        public BinaryFunction(int Source)
        {
            ...
            ValuesArray = Misc.Misc.GetBinaryArray(Source, (int) Math.Ceiling(Math.Log(Source, 2)));
        }
        ...
        {
            ValuesArray = Misc.Misc.GetBinaryArray(Source, (int) Math.Pow(2, VariablesCount));
        }
        ...
        public bool[,] TruthTable
        {
            get
            {
                ...
                {
                    bool[] Row = Misc.Misc.GetBinaryArray(i, CountOfVariables);
        ...
        public bool IsTmClass
        {
            get
            {
                ...
                {
                    DNF idnf = new DNF(Misc.Misc.GetBinaryArray(i, CountOfVariables).Select(val => new bool?(val)).ToArray());
                    for (int j = i + 1; j < ValuesArray.Length; j++)
                    {

                        DNF jdnf = new DNF(Misc.Misc.GetBinaryArray(j, CountOfVariables).Select(val => new bool?(val)).ToArray());
            ...
            get
            {
                ...
                for (int i = 0; i < ValuesArray.Length; i++)
                {
                    if (!PascalTriangle[i][0])
                        continue;
                    if (Misc.Misc.GetBinaryArray(i).Count(val => val) > 1)
                        return false;
                }
        ...
        public int Degree
        {
            get
            {
                ...
                return new int[PascalTriangle.Length].Select((val, ind) => ind)
                    .Where(val => PascalTriangle[val][0]).Select(index => Misc.Misc.GetBinaryArray(index).Count(val => val))
                    .Max();
            }
        }
        ...
        public bool[][] VariablesSets
        {
            get
            {
                return Enumerable.Range(0, ValuesArray.Length).Select(set => Misc.Misc.GetBinaryArray(set, CountOfVariables))
                    .ToArray();
            }
        }
        ...
        public string PDNF
        {
            get
            {
                ...
                for (byte i = 0; i < ValuesArray.Length; i++)
                {
                    ...
                    bool[] variables = Misc.Misc.GetBinaryArray(i, CountOfVariables);
        ...
        public string MDNF
        {
            get
            {
                ...
                for (byte i = 0; i < ValuesArray.Length; i++)
                {
                    if (!ValuesArray[i])
                        continue;
                    PDNF.Add(Misc.Misc.GetBinaryArray(i, CountOfVariables).Select(elem => (bool?)elem).ToList());
                }
