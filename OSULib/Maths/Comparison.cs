...
namespace OSULib.Maths
{
    ...
    public static class Comparison
    {
        ...
        public static Dictionary<int, int> Factor(int n)
        {
            ...
            while (n > Misc.Misc.Data.PrimeNumbers.Last() ||
                   !Misc.Misc.Data.PrimeNumbers.Contains((ushort) n))
            {
                bool DividerFound = false;
                foreach (ushort PrimeNumber in Misc.Misc.Data.PrimeNumbers)
                {
                    if (n % PrimeNumber == 0)
                    {
                        n /= PrimeNumber;
        ...
        public static class MultiplicativeInverse
        {
            ...
            public static LinearComparison BinaryPowLinearComparison(LinearComparison source, BigInteger degree)
            {
                ...
                bool[] sma = Misc.Misc.GetBinaryArray(degree).Skip(1).ToArray();
