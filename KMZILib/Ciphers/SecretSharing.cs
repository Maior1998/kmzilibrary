    ...
    public static partial class Ciphers
    {
        ...
        public static class SecretSharing
        {
            private static BigInteger GetBiggerRandomPrime(BigInteger current)
            {
                ...
                while (SSPTFull(find) == PrimalityTestResult.Composite)
                    find += 2;
                return find;
            }
                ...
                public static class AsmuthBloomScheme
                {
                    ...
                    public static IEnumerable<CRTPart> Share(BigInteger Key, int Count, int Limit = 0)
                    {
                        ...
                        */
                        ...
                        while (true)
                        {
                            ...
                            List<CRTPart> Result = primes.Select(prime => new CRTPart() {Comparison = new LinearComparison(NewM, prime), P = p}).ToList();
                            return Result;
                        }
                    }
                    ...
                    public static BigInteger Restore(List<CRTPart> Parts, int Limit)
                    {
                        if(Parts.Count<Limit)
                            throw new InvalidOperationException("Невозможно восстановить секрет: число долей меньше порогового значения");
