...
namespace KMZILib
{
    public static partial class Ciphers
    {
        ...
        public static class TranspositionCipher
        {
            ...
            public class TCK
            {
                ...
                public TCK(string TextKey)
                {
                    ...
                    if(Rows.Length!=Languages.CurrentLanguage.Alphabet.Length)
                        throw new InvalidOperationException("Размер ключа не совпадает с длиной алфавита.");
                    Regex KeyRegex = new Regex(string.Concat(@"\s*([",Languages.CurrentLanguage.Alphabet, @"])\s*->\s*([", Languages.CurrentLanguage.Alphabet, @"])\n?"));
                    ...
                    foreach (string Row in Rows)
                    {
                        ...
                        TranspositionMatrixSorted.Add(
                            (byte)Languages.CurrentLanguage.Alphabet.IndexOf(match.Groups[1].Value, StringComparison.Ordinal),
                            (byte)Languages.CurrentLanguage.Alphabet.IndexOf(match.Groups[2].Value, StringComparison.Ordinal));
                    }
            ...
            {
                byte[] Result = new byte[Languages.CurrentLanguage.Alphabet.Length].Select((val, ind) => (byte)ind).ToArray();
