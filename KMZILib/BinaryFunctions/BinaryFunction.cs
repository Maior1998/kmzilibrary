    ...
    public partial class BinaryFunction
    {
        ...
        public bool IsTmClass
        {
            get
            {
                ...
                {
                    bool[] ibool = GetBinaryArray(i, CountOfVariables);
                    bool[] jbool = GetBinaryArray(j, CountOfVariables);
                    if(!IsPrecede(ibool,jbool)) continue;
        ...
        public string PDNF
        {
            get
            {
                ...
                for (byte i = 0; i < ValuesArray.Length; i++)
                {
                    ...
                    for (int j = 0; j <CountOfVariables; j++)
                        buffer.Add(variables[j] ? VariablesAlphabet[j].ToString():$"!{VariablesAlphabet[j].ToString()}");
            ...
            get
            {
                ...
                foreach (DNF implicant in BufferDNFs)
                {
                    ...
                    foreach (DNF BaseDNF in BaseSDNFs)
                        QuineTable[implicant].Add(BaseDNF,IsPrecedeDNF(implicant,BaseDNF));
                }
                ...
                while (QuineTable.First().Value.Count != 0)
                {
                    ...
                    DNF BestRow = CandidatesRows.First(row => row.TrueCount == CandidatesRows.Max(comp => comp.TrueCount));
                    Result.Add(BestRow);
        ...
        /// <summary>
        /// Получение числа точек в столбце
        /// </summary>
        /// <param name="QuineTable"></param>
        ...
        private static bool IsPrecedeDNF(DNF First, DNF Second)
        {
            //00-0 : 0000 0010
            for (int i = 0; i < First.Length; i++)
            {
                if(!First[i].HasValue) continue;
        ...
        private class DNF : IEquatable<DNF>
        {
            ...
            public bool?[] Values;
            public bool IsParticiated=false;
        ...
        /// <summary>
        /// Строковое представление функции в виде полиндрома Жегалкина
        /// </summary>
        ...
        /// <summary>
        /// Метод, возвращающий значение целого числа в виде массива двоичных значений в указанном количество от его начала (младших бит)
        /// </summary>
        ...
        /// <summary>
        /// Определяет, предшествует ли один набор двоичных значений другому набору.
        /// </summary>
        /// <param name="First">Первый предположительно предшествующий набор значений</param>
        /// <param name="Second">Второй набор логических значений, которому предположительно должен предшествовать первый набор</param>
        /// <returns>trur - Набор предшествует. false - не предшествует.</returns>
        private static bool IsPrecede(bool[] First, bool[] Second)
        {
            return !First.Where((t, i) => t && !Second[i]).Any();
        }

        /// <summary>
        /// Метод, возвращающий таблицу Поста для заданного множества функций. Строки - функции. Столбцы - класс функций T0, T1, Ts, Tm, Tl соответственно
        /// </summary>
        ...
        /// <summary>
        /// Для заданной таблице Поста вычисляет, является ли множество функций, по которой составлена таблица, функционально полным
        /// </summary>
